...
describe('Trigger semantics (verify the trigger lets through what it should)', () => {
  ...
  it('diver PATCH emergency contact → 204 (regression — non-privileged column still writable)', async () => {
    const r = await rawFetch(restUrl(`/profiles?id=eq.${diverA.id}`), {
      method: 'PATCH',
      headers: diverHeaders(),
      body: { emergency_contact_name: 'wire-probe-edit' },
    })
    expect([200, 204]).toContain(r.status)
    const after = await admin.from('profiles').select('emergency_contact_name').eq('id', diverA.id).single()
    expect(after.data?.emergency_contact_name).toBe('wire-probe-edit')
  })

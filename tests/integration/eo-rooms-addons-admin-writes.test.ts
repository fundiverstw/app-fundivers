...
describe('EO_rooms admin writes', () => {
  it('admin can insert', async () => {
    ...
    const { error } = await sb.from('EO_rooms' as never).insert({
      _id: id, display_name: 'Test Twin', title: 'twin',
    } as never)
  ...
  it('admin can update', async () => {
    ...
    const id = crypto.randomUUID()
    createdRoomIds.push(id)
    await admin.from('EO_rooms' as never).insert({ _id: id, display_name: 'pre' } as never)
    const { error } = await sb.from('EO_rooms' as never).update({ display_name: 'post' } as never).eq('_id', id)
    expect(error).toBeNull()
    const { data } = await admin.from('EO_rooms' as never).select('display_name').eq('_id', id).single<{ display_name: string }>()
    expect(data?.display_name).toBe('post')
  })

  it('admin can delete', async () => {
    const sb = await userClient(adminUser.email, adminUser.password)
    const id = crypto.randomUUID()
    await admin.from('EO_rooms' as never).insert({ _id: id, display_name: 'doomed' } as never)
  ...
  it('diver cannot insert', async () => {
    ...
    const id = crypto.randomUUID()
    const { error } = await sb.from('EO_rooms' as never).insert({
      _id: id, display_name: 'diver tried',
    } as never)
  ...
  it('diver cannot update', async () => {
    ...
    const id = crypto.randomUUID()
    createdRoomIds.push(id)
    await admin.from('EO_rooms' as never).insert({ _id: id, display_name: 'before' } as never)
    const { error, count } = await sb
      .from('EO_rooms' as never)
      .update({ display_name: 'after' } as never, { count: 'exact' })
      .eq('_id', id)
  ...
  it('diver cannot delete', async () => {
    ...
    const id = crypto.randomUUID()
    createdRoomIds.push(id)
    await admin.from('EO_rooms' as never).insert({ _id: id, display_name: 'survives' } as never)
...
describe('Other_Addons admin writes', () => {
  it('admin can insert', async () => {
    ...
    const { error } = await sb.from('Other_Addons' as never).insert({
      _id: id, display_name: 'Test SMB', title: 'smb',
    } as never)
    expect(error).toBeNull()
  })
  ...
  it('admin can update', async () => {
    ...
    const id = crypto.randomUUID()
    createdAddonIds.push(id)
    await admin.from('Other_Addons' as never).insert({ _id: id, display_name: 'pre' } as never)
    const { error } = await sb.from('Other_Addons' as never).update({ display_name: 'post' } as never).eq('_id', id)
    expect(error).toBeNull()
    const { data } = await admin.from('Other_Addons' as never).select('display_name').eq('_id', id).single<{ display_name: string }>()
    expect(data?.display_name).toBe('post')
  })
  ...
  it('admin can delete', async () => {
    const sb = await userClient(adminUser.email, adminUser.password)
    const id = crypto.randomUUID()
    await admin.from('Other_Addons' as never).insert({ _id: id, display_name: 'doomed' } as never)
  ...
  it('diver cannot update', async () => {
    ...
    const id = crypto.randomUUID()
    createdAddonIds.push(id)
    await admin.from('Other_Addons' as never).insert({ _id: id, display_name: 'before' } as never)
    const { error, count } = await sb
  ...
  it('diver cannot delete', async () => {
    ...
    const id = crypto.randomUUID()
    createdAddonIds.push(id)
    await admin.from('Other_Addons' as never).insert({ _id: id, display_name: 'survives' } as never)

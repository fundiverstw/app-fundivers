...
describe('profiles RLS', () => {
  ...
  it('diver can update own emergency contact, not another diver', async () => {
    const sb = await userClient(diverA.email, diverA.password)
    const ok = await sb.from('profiles').update({ emergency_contact_name: 'A-self' }).eq('id', diverA.id).select().single()
    expect(ok.error).toBeNull()
    expect(ok.data?.emergency_contact_name).toBe('A-self')
    const fail = await sb.from('profiles').update({ emergency_contact_name: 'hacked' }).eq('id', diverB.id).select()
    // RLS silently filters the row out — update touches zero rows.
    expect(fail.data ?? []).toEqual([])
    const after = await admin.from('profiles').select('emergency_contact_name').eq('id', diverB.id).single()
    expect(after.data?.emergency_contact_name).not.toBe('hacked')
  })

  it('admin can update any diver profile (full edit control)', async () => {
    const sb = await userClient(adminUser.email, adminUser.password)
    const ok = await sb.from('profiles').update({
      emergency_contact_name: 'Admin-edited',
      nationality: 'Atlantean',
      cert_level: 'Rescue Diver',
    }).eq('id', diverB.id).select().single()
    expect(ok.error).toBeNull()
    expect(ok.data?.emergency_contact_name).toBe('Admin-edited')

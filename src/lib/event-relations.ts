...
// event_addons, event_destinations, event_discounts) — the single source of
// truth. These helpers load them for editing/preload and write them back
// transactionally via the set_event_relations RPC. Courses simply carry no
// rooms/destinations.

/** Load an event's room / add-on / destination / discount ids. */
export async function fetchEventRelations(id: string): Promise<EventRelations> {
  const [rooms, addons, dests, discounts] = await Promise.all([
...
export async function saveEventRelations(id: string, rels: EventRelations) {
  const { error } = await supabase.rpc('set_event_relations', {
    ...
    p_discount_ids: rels.discountIds,
  })
  return error
}

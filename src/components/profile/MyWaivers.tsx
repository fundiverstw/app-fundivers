...
export function MyWaivers({ diverId }: { diverId: string }) {
  ...
  return (
    <section className="bg-white/70 backdrop-blur-md border border-sky-200 rounded-xl p-4 space-y-3">
      <div>
        <h2 className="text-sm font-bold text-blue-900">My Waivers</h2>
        <p className="text-xs text-blue-950/70 font-medium">
          Annual forms required before you dive. Course-specific waivers are signed when you register.
        </p>
      </div>
      {signatures === null ? (
        <p className="text-xs text-blue-950/70 font-medium italic">Loading…</p>
      ) : (
  ...
  return (
    <li className="py-2 flex items-center justify-between gap-3">
      <span className="min-w-0">
        <span className="block text-sm text-blue-900 font-medium truncate">{def.title}</span>
        <span className={`block text-xs font-medium ${STATUS_CLASS[status.state]}`}>
          {STATUS_LABEL[status.state]}
          {ok && status.validUntil && <span className="text-blue-950/70"> · valid until {fmtDate(status.validUntil)}</span>}
        </span>
      </span>
      <button
        type="button"
        onClick={onSign}
        className="shrink-0 px-3 py-1 rounded-lg bg-blue-900 hover:bg-blue-950 text-white text-xs font-semibold"
      >
        {ok ? 'Re-sign' : 'Sign'}
      </button>
    </li>

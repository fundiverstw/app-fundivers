    ...
    <div className="max-w-2xl mx-auto space-y-4">
      ...
      {loading ? (
        ...
        <ul className="space-y-2">
          {vehicles.map(v => (
            <li key={v.id} className="bg-white/70 backdrop-blur-md border border-sky-200 rounded-xl p-3 flex items-start justify-between gap-3">
              <div className="min-w-0">
                <p className="font-medium text-blue-900 text-sm truncate">
                  {v.name}{!v.active && <span className="ml-2 text-xs text-blue-900/60">(retired)</span>}
                </p>
  ...
  return (
    <Modal labelledBy="vehicle-form-title" onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-3">
        <h2 id="vehicle-form-title" className="text-lg font-bold text-blue-900">{vehicle ? 'Edit vehicle' : 'New vehicle'}</h2>
        <Labelled label="Name *">
        ...
        <label className="flex items-center gap-2 text-sm text-blue-900">
          <input type="checkbox" checked={active} onChange={e => setActive(e.target.checked)} className="accent-blue-900" />
          In service (counts toward ride planning)
        </label>
        <div className="flex justify-end gap-2 pt-1">
          <button type="button" onClick={onClose} className="text-sm font-semibold text-blue-900 px-3 py-1.5">Cancel</button>
          <button type="submit" disabled={submitting}
            className="text-sm font-semibold bg-blue-900 hover:bg-blue-950 disabled:opacity-50 text-white px-4 py-1.5 rounded-lg">
...
function ConfirmModal({
  ...
  return (
    <Modal labelledBy="vehicle-confirm-title" onClose={onClose}>
      <h2 id="vehicle-confirm-title" className="text-lg font-bold text-blue-900">{title}</h2>
      <p className="text-sm text-blue-900/80">{body}</p>
      <div className="flex justify-end gap-2 pt-1">
        <button type="button" onClick={onClose} className="text-sm font-semibold text-blue-900 px-3 py-1.5">Cancel</button>

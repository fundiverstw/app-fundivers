...
describe('AdminDutyPage', () => {
  it('shows the logged-in admin their own upcoming duties + flags unstaffed events', async () => {
    ...
    const admins = [
      { id: 'admin-1', display_name: 'Ada', full_name: 'Ada Lovelace', role: 'admin' },
      { id: 'admin-2', display_name: 'Grace', full_name: 'Grace Hopper', role: 'admin' },
    ]
    const events = [
  ...
  it('does not render the "your duties" section when the user has none', async () => {
    ...
    from.mockImplementation((table: string) => {
      ...
      if (table === 'profiles') return mockQueryBuilder({ data: [{ id: 'admin-1', role: 'admin', display_name: 'Ada', full_name: 'Ada Lovelace' }] })
      return mockQueryBuilder({ data: [] })
    })

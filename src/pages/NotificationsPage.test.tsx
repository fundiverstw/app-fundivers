...
describe('NotificationsPage', () => {
  ...
  it('renders ASCII-art rows with a clamp(...) font-size and no overflow scroll so 80×30 fits the card on phones', async () => {
    setup([{
      id: 'n-art', user_id: 'u',
      title: 'Banner', body: '##############\n# DIVE TODAY #\n##############',
      url: null, kind: 'broadcast',
    ...
    const user = userEvent.setup()
    renderPage()
    await user.click(await screen.findByRole('button', { name: /banner/i }))

    // The pre carries data-ascii-art="true" as the marker that the
    // ascii-art branch ran. We assert that + the absence of overflow-x
    // (the whole point of the mode is to fit, not scroll). The actual
    // clamp(...) + cqi font-size is set inline; we don't assert it
    // here because happy-dom strips CSS values it can't parse.
    const pre = document.querySelector('pre[data-ascii-art="true"]')
    expect(pre).not.toBeNull()
    expect(pre!.className).not.toMatch(/overflow-x/)
  })

  it('falls back to normal-size monospace + horizontal scroll when is_ascii_art is false (default)', async () => {
    setup([{
      id: 'n-plain', user_id: 'u',
      title: 'Plain', body: 'Just a regular message.',
      url: null, kind: 'broadcast',
      event_id: null,
      created_at: '2026-05-05T00:00:00.000Z',
      read_at: null,
    ...
    const user = userEvent.setup()
    renderPage()
    await user.click(await screen.findByRole('button', { name: /plain/i }))
    // The pre exists but does NOT carry the ascii-art marker on the
    // default branch — and it keeps overflow-x-auto for long lines.
    const pre = document.querySelector('pre')!
    expect(pre.getAttribute('data-ascii-art')).toBeNull()

...
| `travel_destinations` | `id`, `admin_title`, `country`, `divetype`, `international`, … | Dive-location catalog (Green Island, Palau…). Events are tagged with these via the `event_destinations` junction (EventForm picker); `divetype` drives calendar local-vs-trip coloring ('Shore Diving' = local/green). Admin-managed at `/admin/destinations`. |
...
| `almanac_records` | `id`, `diver_id`, `site_id`, `obs_date`, condition columns, `status`, `approved_by/_at`, `staff_notes` | Crowdsourced conditions at a site on a day — temperatures, visibility, current, wave, weather, coral, plus terrain readings for the kinds that answer `hasTerrainConditions`. One record per `(site_id, obs_date, diver_id)`; the page reads them back **by calendar date**, so every site observed on a day sits in one bucket. `authenticated` holds **SELECT only**: a diver files through `submit_almanac_record()` (always lands on `pending`, revisable only while pending), staff rule through `moderate_almanac_record()`, and the crowd reads approved rows through `almanac_records_in_range()`. Divers see their own rows plus every approved one; staff see the queue via `almanac_pending_records()`. A diver reads their own back on the **Your entries** tab of `/almanac` — every entry, with its readings written out — and may correct one **until staff rule on it**: `submit_almanac_record()` refuses a reviewed record, so the row offers Edit only while pending and says why otherwise. The form is seeded from the record for that edit, because the RPC writes every column and a form filled from half a record would blank the other half on save; site and date are held still, since they are what identify it. `withdraw_almanac_record()` deletes a pending record of the caller's own — pending and own are both checked in the function, `security definer` running past RLS — and is how a diver takes back one filed against the wrong site or day. The FK to `dive_sites` is `ON DELETE RESTRICT` — a site that carries observations is retired, not deleted. **What was seen is not on this row**: wildlife lives in `almanac_sightings`, one row per animal, and the read RPCs return `wildlife_taxa uuid[]` plus `wildlife_unmatched text[]` in place of the old free-text `wildlife` column. |
| `taxa` / `taxon_names` / `almanac_sightings` | taxa: `id`, `rank`, `scientific_name`, `authority`, `worms_aphia_id`, `parent_id`, `accepted_id`, `status`, `proposed_by`, `reviewed_by/_at`, `staff_notes`. names: `taxon_id`, `lang`, `name`, `is_primary`. sighting: `record_id`, `taxon_id`, `raw_label` | The wildlife catalog, and the reason one animal is one thing however it is spelled or whatever language it is spelled in. **`scientific_name` is the root key** — unique on `lower()`, shaped by rank (a binomial for a species, one capitalized word above it), with `parent_id` chaining the ranks (`phylum` … `species`, subclass and superorder included so "a shark" and "a ray" have honest nodes) under a trigger that refuses a parent not strictly above its child. Every vernacular name is a `taxon_names` row, and **`(lang, lower(name))` is unique across the table**: within one language a common name means exactly one animal. A name that turns out to be a synonym keeps its row with `accepted_id` pointing at the taxon that owns the sightings, and writes resolve through it, so two names cannot split a tally. `authenticated` holds **SELECT only**: divers file proposals through `propose_taxon()` (get-or-propose, always `pending`), staff curate through `save_taxon()` / `moderate_taxon()` (which also **merges**: sightings move, the loser becomes a synonym) / `set_taxon_name()`, and the pre-catalog free text sits in `almanac_sightings.raw_label` until `map_unmatched_wildlife()` points it at a taxon. A record naming an unreviewed taxon cannot be approved. See [wildlife-taxa.md](./wildlife-taxa.md). |
| `coral_surveys` / `coral_survey_colonies` | survey: `id`, `diver_id`, `site_id`, `surveyed_on`, `surveyed_at`, `depth_m`, `water_temp_c`, `survey_method`, `transect_length_m`, `notes`, `status`, `approved_by/_at`, `staff_notes`. colony: `survey_id`, `ordinal`, `coral_type`, `lightest_hue/_level`, `darkest_hue/_level`, `diameter_cm` | A CoralWatch Coral Health Chart survey: a header saying when, where and under what conditions, plus one row per colony matched to the printed chart at its palest and darkest shade (four hue columns B–E, six lightness levels; 1 is bleached, 6 fully pigmented). Distinct from `almanac_records.coral_health`, which is one ordinal judgment per site-day and carries no colony count, depth or temperature. One survey per `(site_id, surveyed_on, diver_id)`. `authenticated` holds **SELECT only**: `submit_coral_survey()` files or revises (colonies replaced wholesale, never merged), `moderate_coral_survey()` rules, `coral_surveys_in_range()` reads approved surveys with colonies aggregated in, `coral_pending_surveys()` is the staff queue. `darkest_level >= lightest_level` is a check constraint — the chart is read palest first, so the reverse is a transposed pair. Colonies cascade with their survey; the FK to `dive_sites` is RESTRICT. Arithmetic (mean shade, bleached fraction) lives in `src/lib/coral-survey.ts`. |
